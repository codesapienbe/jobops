...
export type SupportedLanguage = 'en' | 'nl' | 'fr' | 'tr';
...
class I18nManager {
  ...
  private readonly TRANSLATION_API_URL = 'https://libretranslate.de/translate';
  private readonly FALLBACK_API_URL = 'https://translate.argosopentech.com/translate';
  ...
  private async loadAllTranslations(): Promise<void> {
    const languages: SupportedLanguage[] = ['en', 'nl', 'fr', 'tr'];
    ...
    const loadPromises = languages.map(async (lang) => {
      try {
        ...
        const translationData: TranslationData = await response.json();
   ...
   */
  t(key: string, section: keyof TranslationData = 'ui'): string {
    ...
    const recordData = sectionData as Record<string, string>;
   ...
   */
  private updateConsoleMessages(): void {
   ...
   */
  getNotificationMessage(key: string): string {
   ...
   * Get console message
   */
  getConsoleMessage(key: string): string {
   ...
   */
  async translateDynamicContent(): Promise<void> {
