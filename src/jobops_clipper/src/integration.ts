import { LinearClient, LinearIssue, LinearTeam, LinearProject, LinearLabel } from './client';
import { LinearTask, LinearSubtask, LinearSectionMapping } from './models';
...
export class LinearIntegrationService {
  ...
  private dataManager: JobOpsDataManager;

  constructor(config: LinearIntegrationConfig, dataManager: JobOpsDataManager) {
    this.config = config;
    this.client = new LinearClient(config.apiKey);
    this.dataManager = dataManager;
  }

  private getSectionMappings(): LinearSectionMapping[] {
    return [
      {
  ...
  private formatSectionContent(sectionData: any, sectionName: string): string {
  ...
  private async getLabelIds(labels: string[], teamId: string): Promise<string[]> {
    try {
  ...
  async exportJobToLinear(jobApplicationId: string): Promise<LinearExportResult> {
    try {
...
*Created by JobOps Clipper Extension*
      ...
      if (this.config.autoCreateSubtasks) {
        for (const mapping of sectionMappings) {
          const sectionData = (jobData as any)[mapping.sectionName];
          const sectionContent = this.formatSectionContent(sectionData, mapping.sectionName);

          const subtask: LinearSubtask = {
